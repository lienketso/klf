...
<div class="thanhtonghop">
     <div class="quantrong"><img src="{$URL_IMAGES}/14.png" /></div>
       ...
       <div class="thanhxuly">
        ...
        <div class="t1"> 
        <a href="{$ADMIN_URL}/?mod={$mod}&category_id=0" {if $category_id!=''} style="color:#F00" {/if}>Hiển thị theo nhóm</a>
        </div>
        <div class="t3">
...
<input type="hidden" name="btnSave" id="btnSave" value="">
          ...
          <tr class="tr_chinh">
            ...
            <td width="14%">Tên nhóm tour</td>
            <td width="20%">Cat-name</td>
            <td width="20%">Nhóm cha</td>
          ...
          <tr>
            <td><input type="checkbox" name="{$recond[co].category_id}" /></td>
            <td><a href="{$ADMIN_URL}/?mod={$mod}&act=add&id={$recond[co].category_id}&lang={$recond[co].lang_code}">{$recond[co].name}</a></td>
            <td>{$recond[co].cat_name}</td>
            <td><select name="{$recond[co].category_id}_parent" class="select_thuong">
            <option value="0">- - -</option>
           {$clsMod->option(0,1,4,$recond[co].parent)}
            
            </select></td>
           
            <td><input type="text" size="2" name="{$recond[co].category_id}_is_order" value="{$recond[co].is_order}"></td></td>
            <td>
            <select name="{$recond[co].category_id}_is_online">
              {$clsCore->yes_no(1,$recond[co].is_online)}
            </select>
            </td>
            {if $cat_id!=''}<td><a href="{$ADMIN_URL}/?mod={$mod}&cat_id={$recond[co].category_id}">Xem con</a></td>{/if}
          </tr>
           {/section}
